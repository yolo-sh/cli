...
//go:generate mockgen -destination=../mocks/aws_user_config_env_vars_resolver.go -package=mocks -mock_names UserConfigEnvVarsResolver=AWSUserConfigEnvVarsResolver github.com/yolo-sh/cli/internal/aws UserConfigEnvVarsResolver
type UserConfigEnvVarsResolver interface {
	Resolve() (*userconfig.Config, error)
}

//go:generate mockgen -destination=../mocks/aws_user_config_files_resolver.go -package=mocks -mock_names UserConfigFilesResolver=AWSUserConfigFilesResolver github.com/yolo-sh/cli/internal/aws UserConfigFilesResolver
